...
while(!gameOver)
{
	...
	{	
		...
		if(p1.checkDeadwood() <= 10)
		{
			...
			if(userInput == "Yes")
			{
				if(p1.checkDeadwood() == 0)	//SCORES IF PLAYER REACHES GIN.
				{
					cout << "You have reached Gin. Let's take the score." << endl;
					PLAYER_SCORE += cpu.checkDeadwood() + 20;
					gameOver = true;
					break;
				}
				else	//SCORE IF PLAYER KNOCKS.
				{
					PLAYER_SCORE = PLAYER_SCORE + abs(p1.checkDeadwood() - cpu.checkDeadwood()); 
				}
			}
			else
			{
	...
	{	
		...
		if(cpu.checkDeadwood() <= 10)	//If the CPU's Deadwood is less than ten then it will knock.
		{
			...
			{	
				...
				CPU_SCORE += p1.checkDeadwood() + 20;	//SCORES IF CPU REACHES GIN.
				gameOver = true;
				//break;
			}
			else
			...
			CPU_SCORE = CPU_SCORE + abs(cpu.checkDeadwood() - p1.checkDeadwood());	//SCORES IF CPU KNOCKS
			gameOver = true;
			//break;
			}
		}
